...
async function validateLicense(inputKeyOrEmail, deviceId, productId, linkEmail = null) {
  ...
  if (!productFingerprints.includes(deviceId)) {
    if (productFingerprints.length >= 5) { // Tối đa 5 thiết bị PER PRODUCT để bớt khắt khe
      return { valid: false, message: `Mã kích hoạt đã đạt giới hạn thiết bị cho ${productId} (Tối đa 5).` };
    } else {
